...
struct StatementCompiler : public boost::static_visitor<> {
    ...
    //TODO Move partially (or totally) to Utils
    ...
    void compare_binary(T& if_){
        ...
        if(auto* ptr = boost::get<int>(&if_->arg1)){
            ...
            auto reg2 = manager.get_reg(ltac::get_variable(*if_->arg2));
            ...
            auto reg1 = manager.get_reg(ltac::get_variable(if_->arg1));

            //The basic block must be ended before the jump
            end_basic_block();

            ltac::add_instruction(function, ltac::Operator::CMP_INT, reg1, to_arg(*if_->arg2));
        }
    }
    ...
    void compare_float_binary(T& if_){
        ...
        if(isVariable(if_->arg1) && isVariable(*if_->arg2)){
            ...
            auto reg1 = manager.get_float_reg(ltac::get_variable(if_->arg1));
            auto reg2 = manager.get_float_reg(ltac::get_variable(*if_->arg2));
    ...
    void compare_unary(T& if_){
        if(auto* ptr = boost::get<int>(&if_->arg1)){
            ...
            auto reg = manager.get_reg(ltac::get_variable(if_->arg1));
    ...
    for(; iter != end; iter++){
        ...
        if(var->type().isArray() && var->position().isStack()){
            ...
            if(var->type().base() == BaseType::INT){
                ltac::add_instruction(target_function, ltac::Operator::MEMSET, ltac::Address(ltac::BP, position - 8), static_cast<int>(var->type().size()));
            } else if(var->type().base() == BaseType::STRING){
                ltac::add_instruction(target_function, ltac::Operator::MEMSET, ltac::Address(ltac::BP, position - 8), static_cast<int>(2 * var->type().size()));
            }
        }
    }
